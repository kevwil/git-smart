...
GitSmart.register 'smart-pull' do |repo, args|
  ...
  warn "Ignoring arguments: #{args.inspect}" unless args.empty?
  ...
  failure("Upstream branch '#{upstream_branch}' doesn't exist!") unless repo.exists?(upstream_branch)
  ...
  if head == remote
    ...
    unless new_commits_locally.empty?
    ...
    if merge_base == remote
      ...
      if merge_base == head
        ...
        #
        # When we rebase, we use `git rebase  --rebase-merges`, which attempts to recreate merges
        ...
        note "Both local and remote branches have moved on. Branch '#{branch}' needs to be rebased onto '#{upstream_branch}'"
